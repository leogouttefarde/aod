...
void Solver::display_solution () {
    ...
    for (list<Op>::iterator it = op_list.begin() ;
        ...
        default: // DEST
            if (*it == DEST+1) // Simple destruction
                cout << "d " << i+1 << endl;
            else
...
void Solver::compute_line (int j) {
    int best_k = 0; // Used for the multidestruction
    ...
    for (unsigned int i = 1 ; i <= _source.nb_lines() ; ++i) {
        ...
        /* Comme i augmente, pour stabiliser i-best_k on incremente best_k */
        best_k++;
    }
}
